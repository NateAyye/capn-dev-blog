...
const SignUp: React.FC<SignUpProps> = async ({}) => {
  const providers = (await getProviders()) || [];
  const session = await getServerSession(authOptions);
  if (session) redirect('/');
  return (
    <main className="flex justify-center items-center min-h-screen">
      <div className="flex m-5 ms:m-0 shadow-2xl rounded-lg p-4 flex-col w-full max-w-3xl space-y-6 md:space-y-0 md:space-x-6 md:flex-row">
